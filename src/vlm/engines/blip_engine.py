...
class BLIPEngine:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error(f"Failed to initialize {settings.blip_caption_model}: {e}")
            raise RuntimeError(f"BLIP Engine initialization failed: {e}") from e
        try:
            self._initialize_vqa_model()
        except Exception as e:
            logger.error(f"Failed to initialize {settings.blip_vqa_model}: {e}")
            raise RuntimeError(f"BLIP Engine initialization failed: {e}") from e
    ...
    def _initialize_captioning_model(self):
        ...
        """
        logger.info(f"Loading captioning model: {settings.blip_caption_model}")
        self.caption_processor = BlipProcessor.from_pretrained(settings.blip_caption_model)
        self.caption_model = BlipForConditionalGeneration.from_pretrained(settings.blip_caption_model)
        self.caption_model.to(self.device)
        self.caption_model.eval()

    def _initialize_vqa_model(self):
        """
        Initialize VQA model.
        """
        logger.info(f"Loading VQA model: {settings.blip_vqa_model}")
        self.vqa_processor = BlipProcessor.from_pretrained(settings.blip_vqa_model)
        self.vqa_model = BlipForQuestionAnswering.from_pretrained(settings.blip_vqa_model)
